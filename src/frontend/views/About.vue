          ...
          <ion-car v-if="abouts.length > 0">
                ...
                <ion-slide v-for="about in abouts" :key="about._id"  >
                    <ion-card   style="padding:10px">
                        <ion-card-header>
                            <img :src="about.ImageUrl" style="display: inline;max-width: 200px;">
...
//    VueRecaptcha ,
  ...
  beforeRouteUpdate(to, from, next) {
    if(to.path !== from.path)  this.loadAbout();
    to.hash+from.hash    
    next()
  },
    ...
    methods:{
        ...
        goMenu: function(){
            return this.$router.push({ name: 'Home',params: {isCatering: false, url: this.$store.state.restaurantActive.restaurantUrl} })
        },
